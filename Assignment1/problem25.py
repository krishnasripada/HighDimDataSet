...
for key, value in bernoulliMatrices.items():
    ...
    plt.plot(semiCircleDist.keys(), semiCircleDist.values(), 'ro')
    plt.hist(value, bins=100, normed=1)
    plt.savefig("25_hist_"+str(key)+".png")
    plt.close()
...
for key, value in wignerGaussian.items():
    ...
    plt.plot(semiCircleDist.keys(), semiCircleDist.values(), 'ro')
    plt.hist(value, bins=100, normed=1)
    plt.savefig("25_hist_"+str(key)+"g.png")
    plt.close()
